...
if (! empty($savecat)) {
    ...
    $row['color'] = $nv_Request->get_title('group_color', 'post', '', 1);
    ...
    if (empty($row['title'])) {
        ...
        $sql = "INSERT INTO " . $table_name . " (title, alias, description, keywords, adddefault, numbers, image, color, weight, add_time, edit_time) VALUES (:title, :alias, :description, :keywords, 0, 4, :image, :color, :weight, " . NV_CURRENTTIME . ", " . NV_CURRENTTIME . ")";
        $data_insert = array();
        $data_insert['title'] = $row['title'];
        $data_insert['alias'] = $row['alias'];
        $data_insert['description'] = $row['description'];
        $data_insert['image'] = $row['image'];
        $data_insert['color'] = $row['color'];
        ...
        $stmt = $db->prepare("UPDATE " . $table_name . " SET title= :title, alias = :alias, description= :description, image= :image, color = :color, keywords= :keywords, edit_time=" . NV_CURRENTTIME . " WHERE bid =" . $row['bid']);
...
if ($row['bid'] > 0) {
    list ($row['bid'], $row['title'], $row['alias'], $row['description'], $row['image'], $row['color'], $row['keywords']) = $db->query("SELECT bid, title, alias, description, image, color, keywords FROM " . $table_name . " where bid=" . $row['bid'])->fetch(3);
